    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `Échec de la Suppression<br>du Mod Optionnel ${fullName}`,
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            ...
            if(v.version.major < 9) {
                settingsJavaExecDetails.innerHTML = `Séléctionné: Java ${v.version.major} Mise à jour ${v.version.update} (x${v.arch})${vendor}`
            } else {
                settingsJavaExecDetails.innerHTML = `Séléctionné: Java ${v.version.major}.${v.version.minor}.${v.version.revision} (x${v.arch})${vendor}`
            }
        } else {
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/ZaptoOriginal/AventureLauncher/releases.atom',
        success: (data) => {
